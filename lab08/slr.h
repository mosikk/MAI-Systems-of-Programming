...
public:
    ...
    void add(tState from, const tGramma::tSymbstrset &follow, tGramma::tSymb left, tGramma::tAltind ialt) {
    ...
    // представление недетерминированного конечного автомата
...
public:
    ...
    // конструктор
    tLRI(tGramma::tSymb aleft = 0, tGramma::tAltind aialt = 0, tPoint apoint = 0, tGramma::tSymb asmb = 0)
    ...
    bool operator<(const tLRI &y) const {
        return (smb < y.smb)
               || ((smb == y.smb) && ((left < y.left)
                                      || ((left == y.left) &&
                                          ((ialt < y.ialt) || ((ialt == y.ialt) && (point < y.point))))));
    }
