...
int tParser::parse(const char *source_name) {
    ...
    if (!term) {
        ...
        while (true) {
            ...
            if (next == 0) {
                buf << "Syntax: unmatched token " << gr.decode(term) << "\nexpected: ";
            ...
            if (stack.size() == 2 && left == start && term == 1) { // маркер коца
                lex.End();
                return 0;
            }
        }

    // добавить к сообщению об ошибке номер строки и смещение
    buf << endl;
    buf << setw(4) << lex.GetLineCount() << "|" << lex.GetLineText() << endl;
    buf << "     " << string(lex.GetStartPos(), ' ') << "^" << endl;
