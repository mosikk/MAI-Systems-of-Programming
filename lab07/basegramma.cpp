...
void out_chain(std::ostream &out, const tBaseGramma &gr, const tBaseGramma::tSymbstr &rp) {
    for (size_t i = 0; i < rp.length(); ++i)
        out << " " << gr.decode(rp[i]);
}

void out_prod(std::ostream &out, const tBaseGramma &gr, const tBaseGramma::tRule &rule) {
...
void tBaseGramma::add_to_first(tSymb a, tSymbstr &first) const { // a нетерминал
    for (size_t ialt = 0; ialt < altCount(a); ++ialt)
        addunique(first, rightPart(a, ialt)[0]);
}

void tBaseGramma::createfirst(tSymb a, tSymbstr &first) const {
    first.clear();
    if (terminal(a)) {
        first += a;
        return;
...
void tBaseGramma::add_to_last(tSymb a, tSymbstr &last) const { // a нетерминал
    for (size_t ialt = 0; ialt < altCount(a); ++ialt) {
...
void tBaseGramma::createlast(tSymb a, tSymbstr &last) const {
    last.clear();
...
void tBaseGramma::makeFollow(tSymbstrset &follow) const {
    ...
    // добавить marker в FOLLOW для всех нетерминалов из LAST(start)
